...
# 本地临时目录
tmpdir = '/home/vicent/Workspace/tmp'
# ftp参数
ftp_ip = '10.0.100.134'
ftp_username = 'xmlconvert'
ftp_password = '123456'
ftp_path = '/etl_test'
# 数据库参数
db_uid = 'postgres'
...
def readFile(filename, metadata):
        ...
        if iLen > 0:
            ...
            if sPMDM.find("品") == -1:
                if qforpmdm(sPMDM) == True:
                    tid = uuid.uuid1()
                    sql = "INSERT INTO mrcsb.b2mcczb(id,parent_id,insert_time,farm,sheet,pmdm,dkh,czsj,czr,mcczl,jgccsl,scdd) VALUES('%s','%s','%s','%s','%s','%s','%s','%s','%s',%f,%f,'%s');" % (
                    tid, metadata[0], metadata[1], metadata[2], metadata[3], sPMDM, sDKH.strip(), sCZSJ.strip(),
                    sCZR.strip(), fCZSL, fCCJGSL,sSCDD)
                    sql_l.append(sql)
                    logger.info(sql)
                    # print sql
            else:
    ...
    for row in range(sheet_241.nrows):
        ...
        try:
            ...
            fVAL_1 = 0.0
        # print fVAL_1
        if fVAL_1 > 0 and len(sDbdd)>0:
            # print(row)
            # sPMDM = row_list[1][:4]
            # if sPMDM.find("品") == -1:
            if qforpmdm(sPMDM) == True:
                tid = uuid.uuid1()
                sql = "INSERT INTO mrcsb.b2mrcsb(uid,parent_id,insert_time,farm,pmdm,sheet,jpcdbrk_1,jpcdbrk_2,dbdd) VALUES('%s','%s','%s','%s','%s',%d,%f,'%s','%s');" % (
                    tid, metadata[0], metadata[1], metadata[2], sPMDM, 5, fVAL_1, sVAL_2,sDbdd)
                sql_l.append(sql)
                logger.info(sql)
                    # print ''.join(sql)
            # if sPMDM.find("品") == 1:
    ...
    for row in range(sheet_251.nrows):
        ...
        sDbdd = row_list[6]  #调拨单号
        try:
            if len(sVAL_1) > 0 and len(sDbdd)>0:
                dot = sVAL_1.strip().find('.')
                fVAL_1 = float(sVAL_1[0:dot + 4])
            else:
                fVAL_1 = 0.0
        ...
        if fVAL_1 > 0:
            ...
            if qforpmdm(sPMDM) == True:
                tid = uuid.uuid1()
                sql = "INSERT INTO mrcsb.b2mrcsb(uid,parent_id,insert_time,farm,pmdm,sheet,spcdbrk_1,spcdbrk_2,dbdd) VALUES('%s','%s','%s','%s','%s',%d,%f,'%s','%s');" % (
                    tid, metadata[0], metadata[1], metadata[2], sPMDM, 7, fVAL_1, sVAL_2,sDbdd)
                sql_l.append(sql)
                logger.info(sql)
    ...
    for row in range(sheet_272.nrows):
        ...
        try:
            ...
            fVAL_1 = 0.0
        # print fVAL_1
        if fVAL_1 > 0 and len(sXsdd)>0:
            # print(row)
            if qforpmdm(sPMDM) == True:
                tid = uuid.uuid1()
                sql = "INSERT INTO mrcsb.b2mrcsb(uid,parent_id,insert_time,farm,pmdm,sheet,spcdbck_1, xsdd) VALUES('%s','%s','%s','%s','%s',%d,%f,'%s');" % (
